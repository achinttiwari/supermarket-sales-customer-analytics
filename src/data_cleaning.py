...
def clean_customer_shopping_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in numeric_columns:
        if column in out.columns:
            out[column] = pd.to_numeric(
                out[column].astype("string").str.replace(r"[^0-9.\-]", "", regex=True),
                errors="coerce",
            )

    out["subscription_status"] = _binary_flag(out["subscription_status"])
    out["discount_applied"] = _binary_flag(out["discount_applied"])
    ...
    if "purchase_frequency_days" not in out.columns:
        ...
        out["purchase_frequency_days"] = out["purchase_frequency_days"].fillna(
            out["frequency_of_purchases"].map(frequency_days)
        )
    out["purchase_frequency_days"] = out["purchase_frequency_days"].fillna(
